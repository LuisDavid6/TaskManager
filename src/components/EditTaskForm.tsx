  ...
  return (
    <div className='w-full flex flex-col'>
      <form className='flex flex-col md:flex-row gap-10'>
        ...
        <button
          className='bg-gradient-to-br from-blue-600 to-blue-500 hover:bg-gradient-to-br hover:from-blue-500 hover:to-blue-600 py-2 px-4 text-white rounded-md transition duration-500 ease-in-out hover:scale-110 hover:bg-blue-500 p-2 mt-5 w-full md:w-3/12'
          onClick={async (e) => {
            e.preventDefault()

            if (info.title.length < 1 || info.description.length < 1) setError('Completa todos los campos')
            else {
              updateTask(id, { title: info.title, description: info.description })
              setIsOpen(false)
              setError('')
            }
          }}
